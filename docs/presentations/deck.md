...
Status: Pass 1 — words-only mockup
...
Speaker notes: blockquote prefixed with "Speaker notes:" — Marp/reveal syntax later
...
[HERO IMAGE: title-slide art — a stylized single packet in flight against a backdrop that hints at Google's global backbone. Muted, cinematic. WarGames-era CRT green glow optional.]
...
He hacked the door lock on the way in. Now he needs to SSH into his own GCE VM in `us-east1`. Every wifi and wired network in the building is presumed watched, logged, and forwarded to somebody who wants a copy.

He is authorized. The network is hostile.

[HERO IMAGE: still or illustration of the NORAD infirmary from WarGames — David at a CRT terminal, ideally with the door-lock keypad visible in the corner. If a real still is off-limits, a stylized reproduction: 1983 CRT green-on-black, keypad, "Restricted Access" signage.]

> **Speaker notes:** The setup is deliberately anachronistic — GCE didn't exist in 1983 — and that is the joke. The point survives the anachronism: on ANY untrusted network, the security problem is identical. Coffee shop wifi. Hotel wifi. Foreign airport. A conference. A military facility with monitors on every cable. BeyondCorp's whole thesis is: assume the network is hostile, and design accordingly. Everything after this slide is us doing that.
...
[DIAGRAM: side-by-side visual — left panel a medieval castle-with-moat with attackers scaling the wall; right panel a "no wall" diagram where every request meets a guard checking papers. Same underlying architecture; radically different failure modes.]
...
> **Speaker notes:** Aurora is worth naming out loud because it makes zero trust concrete. This is not an academic paper. This is: Google was breached, decided the network model was the problem, and rebuilt around identity. Every principle of BeyondCorp — no implicit trust from network location, access granted per-request based on who + device + context, least privilege continuously re-evaluated — traces back to that response. IAP is that response, as a service you consume.
...
[DIAGRAM: VPC boundary drawn as a solid wall. One tiny door labeled "35.235.240.0/20 : tcp/22". Every other side of the wall labeled "DENY". Inside the wall: the VM.]
...
[DIAGRAM: The VM with two gates in series. First gate: "IAP tunnel — is this identity allowed here?". Second gate: "sshd + OS Login — does this identity have a valid key?". Both gates must open for a shell to spawn.]
...
> **Speaker notes:** Nothing exotic on the return. The tunnel is bidirectional; a single TLS connection carries traffic both ways. The observer at NORAD watches encrypted HTTPS in both directions and can decode neither. What sits on David's terminal is a fully interactive shell prompt.
...
> **Speaker notes:** This is the punchline for the hero sequence. The datacenter-think version of this architecture would have had at least three publicly-attackable surfaces — VPN endpoint, bastion, sometimes the VM itself. Cloud-think has zero. The attack surface is *nothing*. There is nothing for an attacker to scan. That is the whole thesis in one slide.
...
> **Speaker notes:** Same IAP that let David's SSH tunnel through when he had the right identity. On the wiki, IAP is enforcing a Google Workspace domain restriction. Same mechanism, different rule. Different workload, same layer.
...
> **Speaker notes:** The mapping is direct. Layer 4 identity: IAP + IAM. Layer 5 workload perimeter: Cloud Run ingress lockdown, Cloud SQL private IP, minimal service account. Same shape as David's `35.235.240.0/20`-only firewall + OS Login SSH key check. Different technology; same architectural pattern.
...
[HERO IMAGE: closing art — the Yamato ship silhouetted against a nebula, with the Fort Knox layers superimposed as a translucent shield. Or a callback to the WarGames CRT with the message "SHALL WE PLAY A GAME?" answered by "yes — but properly this time."]

> **Speaker notes:** Land the pitch. The wiki is a proof-of-work artifact — a prospect can visit it right now, watch the NOC, see the honeypots catching bots, read the /docs shelf. This is not a slide deck about what we could build. It is a slide deck built on top of what we did.
